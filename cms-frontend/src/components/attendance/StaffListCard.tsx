                ...
                <div className="space-y-3 max-h-[400px] overflow-y-auto">
                    {todayRecords.length > 0 ? (
                        todayRecords.map((attendance, index) => {
                            const filePath = attendance.sick_leave?.medical_certificate;
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-center justify-between mb-2">
                                        ...
                                        <span
                                            className={`px-2 py-1 rounded-full text-xs font-medium flex items-center space-x-1 ${getTypeColor(
                                    ...
                                    {attendance.type === 'sick_leave' ? (
                                        <div className="space-y-2">
                                            ...
                                            <div className="flex items-center space-x-2">
                                                ...
                                                <span className="text-xs text-gray-700">{formatTime(attendance.createdAt)}</span>
                                            </div>

                                            {/* ✅ LINK FILE */}
                                            {filePath ? (
                                                <a
                                                    href={toFileUrl(filePath)}
                                                    target="_blank"
                                                    rel="noreferrer"
                                                    className="inline-flex items-center gap-1 text-xs font-semibold text-blue-600 hover:text-blue-700 hover:underline"
                                                    title="Buka surat dokter"
                                                >
                                                    <ExternalLink className="h-3 w-3" />
                                                    Lihat Surat Dokter
                                                </a>
                                            ) : (
                                                <span className="text-[11px] text-gray-400">Tidak ada surat dokter</span>
                                            )}
                                        </div>
                                    ) : (
